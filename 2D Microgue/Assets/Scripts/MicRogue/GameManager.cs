...
public class GameManager : MonoBehaviour
{
  ...
  void OnEnable()
    ...
    void ProceedToNextTurn()
    {
        ...
        {
            bool playerHasRemainingTurns = playerEntity.RemainingTurns > 0;

            if (playerHasRemainingTurns)
            { 
                PlayerTurn(playerEntity.RemainingTurns);
                return;
            }
        ...
        else if (currentTurn == Turn.enemies) 
        {
            ...
            bool enemyHasRemainingTurns = currentEnemy.RemainingTurns > 0;

            if (enemyHasRemainingTurns)
            {
                EnemyTurn(currentEnemy, currentEnemy.RemainingTurns);
                return;
            }
    ...
    void PlayerTurn(int _turnsLeft)
    {
        ...
        playerEntity.RemainingTurns = _turnsLeft;
        playerEntity.HasTurn = true;
    ...
    void EnemyTurn(Entity _enemy, int _turnsLeft)
    {
        ...
        _enemy.RemainingTurns = _turnsLeft; // TODO Consolidate this and below into one method
        _enemy.HasTurn = true;

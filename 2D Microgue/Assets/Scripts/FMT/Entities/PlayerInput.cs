...
namespace FMT
{
public class PlayerInput : MonoBehaviour // TODO Implement an interface if trying to add other input methods (pathfinding, etc.)
{
    float inputX, inputY;
    public float InputX => inputX;
    public float InputY => inputY;
    bool disableDiagonalInput = true;
    ...
    void InputKeys()
    {
        ...
        if (disableDiagonalInput && inputX != 0f) { inputY = 0f; }
    }
}
}